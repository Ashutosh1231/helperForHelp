<?php
    ...
    $result = $customer->fetchById($id);
    if($result['status'] == 'success'){
        //print_r($result['data']);
        $customer_name = $result['data'][0]['fname'].' '.$result['data'][0]['mname'].' '.$result['data'][0]['lname'];
    }
    else{
    ...
    $cussubsresult = $customersubscription->fetchByCustomerIdandEndDate($id);
?>
<!doctype html>
<html>
    <head>
    ...
    <body>
        ...
        <div class="block md:flex w-full">
            ...
            <div class="p-3 w-full">
                <div class="w-full max-w-screen mx-auto ">
                    ...
                    <div class="w-full max-w-screen mx-auto ">
                        <div class="w-full">
                            <h1 class="text-2xl text-center mt-6">Details of <?=$customer_name?></h1>
                        </div>
                        <div class='flex'>
                            <!-- Add Button -->
                            <div class='flex py-10'>
                                <button data-modal-target="apply-customer-subscription-modal" class="addcustomersubscriptionbtn text-white  bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm sm:px-3 sm-py-3 sm:text-sm md:px-4 md:py-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" title="Add Subscription for <?=$customer_name;?>">Apply Subscription</button>
                    ...
                    <div class="w-full">
                        <div class='flex justify-between border-solid border-2  border-blue-700 rounded p-3 mb-2'>
                            <div class='flex w-full'>
                                ...
                                <div class='flex-1 px-2'>Plan Name</div>
                                <div class='flex w-28 text-center px-2'><p class="w-full text-center">Price</p></div>
                                <div class='flex w-28 text-center px-2'><p class="w-full text-center">Validity</p></div>
                                <div class='flex w-28 text-center px-2'><p class="w-full text-center">Purchase Date</p></div>
                            </div>   
                        </div>
                        <?php
                            if($cussubsresult['status'] == "success"){
                                foreach($cussubsresult['data'] as $csubsdata){
                                    $cplankey = array_search($csubsdata['subscription_id'],array_column($subs_result['data'],'id'));
                                    ?>
                                    <div class='border-solid border-2  border-slate-200 rounded p-3 mb-2  hover:border-dotted'>
                                        <div class='flex w-full'>
                                            <div class='flex w-18 px-2'><?=$csubsdata['id'];?></div>
                                            <div class='flex-1 px-2'><?=$subs_result['data'][$cplankey]['name'];?></div>
                                            <div class='flex w-28 text-center px-2'><p class="w-full text-center"><?=$csubsdata['total_price'];?></p></div>
                                            <div class='flex w-28 text-center px-2'><p class="w-full text-center"><?=date('d-m-Y',strtotime($csubsdata['end_date']));?></p></div>
                                            <div class='flex w-28 text-center px-2'><p class="w-full text-center"><?= date('d-m-Y',strtotime('+530 minutes', strtotime($csubsdata['ts'])));?></p></div>
                                        </div>   
                                    </div>
                                    <?php        
                                }
                        ...
                        ?>
                        <div class="w-full">
                            <h3 class=" w-full text-center my-6 mx-auto text-xl font-medium underline underline-offset-8">Addresses</h3>
                            <div class='flex'>
                            <!-- Add Button -->
                                <div class='flex pb-10'>
                                    <button data-modal-target="" class="addcustomersubscriptionbtn text-white  bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm sm:px-3 sm-py-3 sm:text-sm md:px-4 md:py-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" title="Add Address for <?=$customer_name;?>'s service area">Add Address</button>
                                        ...
                                        <div class='border-solid border-2  border-slate-200 rounded p-3 mb-2  hover:border-dotted'>
                                            ...
                                            <div class="w-full m-4">
                                                <div class="w-full flex justify-between my-3">
                                                    <button data-modal-target="add-service-variable-modal"  service_city_id="<?=$data['id'];?>" action="add_variable" class="addvariable text-white  bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm sm:px-3 sm-py-3 sm:text-sm md:px-4 md:py-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" title="Add new city to <?=$customer_name;?> service area">Add Service Variable</button>
                                                </div>
                                                <?php
        ...
        <?php
            include_once('includes/apply_customer_subscription_modal.php');
            include_once('includes/footer_links.php');
        ?>
        <script>
            $(document).ready(function(){
                $(".addcustomersubscriptionbtn").click(function(e){
                    e.preventDefault();
                    var id = $(this).attr('customerid');
                    const customersubscriptionmodal = FlowbiteInstances.getInstance('Modal','apply-customer-subscription-modal');
                    customersubscriptionmodal.show();
                });

                $("#acs_city_id").change(function(e){
                    e.preventDefault();
                    var cityid = $(this).val();
                    $.post('customer_save.php',{cityid:cityid,status:'1',action:'fetchsubcriptionbycity'}, function(data){
                        if(data){
                            data = JSON.parse(data);
                            if(data.status == 'success'){
                                for(var i = 0; i < data.data.length; i++){
                                    $("#acs_subscription_plan").append('<option value="'+data.data[i].id+'">'+data.data[i].name+' ('+data.data[i].duration+' months)</option>');
                                    $("#acs_price").val('');
                                    $("#acs_discount").val('');
                                    $("#acs_total").val('');
                                }
                                ...
                                alert("No Subscription Plans avialable for the city.");
                            }                                
                        }
                        else{
                ...
                $("#acs_subscription_plan").change(function(e){
                    e.preventDefault();
                    var planid = $(this).val();
                    $.post('customer_save.php',{planid:planid,action:'fetchsubcriptionplan'}, function(data){
                        if(data){
                            data = JSON.parse(data);
                            if(data.status == 'success'){
                                ...
                                if(!$("#acs_discount").val()){
                                    ...
                                    var discount = parseFloat($("#acs_discount").val());
                                    alert(parseFloat($("#acs_discount").val()))
                                }
                                $("#acs_total").val(price - discount);    
                            }
                        }
                    })
                });

                $("#acs_discount").change(function(e){
                    e.preventDefault();
                    var price =parseFloat($("#acs_price").val());
                    var discount = parseFloat($("#acs_discount").val());
                    $("#acs_total").val(price - discount);
                });
                ...
                $("#customerchangestatus").click(function(e){
                    ...
                    var action = 'customerchangestatus';
