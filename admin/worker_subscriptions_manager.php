<?php
    ...
    $serviceresult = $service->fetchAll();
    ...
    <body>
        ...
        <div class="block md:flex w-full">
            ...
            <div class="p-3 w-full">
                ...
                <div class="w-full max-w-screen px-4 mx-auto lg:px-12">
                    <div class="w-full">
                        <h1 class="text-2xl text-center mt-6">Worker Subscription Plan Manager</h1>
                    </div>
                    ...
                    <div class='flex  justify-between '>
                       ...
                       <!-- Add Button -->
                        <div class='flex py-10 '>
                            <button data-modal-target="add-worker-subscription-modal" id="addwsp" class="text-white  bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm sm:px-3 sm-py-3 sm:text-sm md:px-4 md:py-2 ">Add New</button>
                    ...
                    <div class="w-full">
                        <div class='flex justify-between border-solid border-2  border-blue-700 rounded p-3 mb-2'>
                                ...
                                <div class='flex w-28 text-center'><p class="w-full text-center">Services</p></div>
                            ...
                            if($result['status']=="success"){
                                if(isset($result['data'])){
                                    foreach($result['data'] as $data){
                                        ...
                                        $services = '';
                                        $service_id = explode(',',$data['services']);
                                        $sercounter = 1;
                                        foreach($service_id as $inservice){
                                            $key = array_search($inservice,array_column($serviceresult['data'],'id'));
                                            if($sercounter == count($service_id)){
                                                $services.=$serviceresult['data'][$key]['name'];
                                            }
                                            else{
                                                $services.=$serviceresult['data'][$key]['name'].', ';
                                            }
                                        ...
                                        <div class='flex justify-between border-solid border-2  border-slate-200 rounded p-3 mb-2  hover:border-dotted'>
                                                ...
                                                <div class='flex w-20  mx-auto cursor-pointer'>
                                                    <a href="#" class="w-fulll mx-auto text-center edit" wspid="<?=$data['id'];?>"  title="Click here to edit the worker subscription plan">
                                                ...
                                                <div class=" flex w-20 text-center ">
                                                    <?php
                                                        if($data['status']=='0'){
                                                            ?>
                                                            <a href="#" class="changestatus mx-auto text-center" wspid="<?=$data['id'];?>" cstatus="<?=$data['status'];?>" title="Click here to enable the customer subscription plan">
                                                            ...
                                                            ?>
                                                            <a href="#" class="changestatus mx-auto text-center" wspid="<?=$data['id'];?>" cstatus="<?=$data['status'];?>"  title="Click here to disable the customer subscription">
        ...
        <?php
            include_once('includes/add_worker_subscription_modal.php');
        ...
        <script>
            $(document).ready(function(){
                ...
                $("#addwsp").click(function(e){
                    e.preventDefault();
                    const modal = FlowbiteInstances.getInstance('Modal', 'add-worker-subscription-modal');
                    $("#action").val('add');
                    $("#id").val('');
                    modal.show(); 
                });
                ...
                $(".edit").click(function(e){
                    e.preventDefault();
                    const modal = FlowbiteInstances.getInstance('Modal', 'add-worker-subscription-modal');
                    var id = $(this).attr('wspid');
                    //alert(id);
                    var action = 'fetchById';
                    $.post('worker_subscription_plan_save.php',{id:id,action:action},function(data){
                        //alert(data);
                        if(data){
                            ...
                            if(data.status == 'success'){
                                ...
                                var service = data.data.services.split(',');
                ...
                $(".changestatus").click(function(e){
                    ...
                    $.post('worker_subscription_plan_save.php',{id:id,status:status,action:action}, function(data){
                        if(data){
                            location.reload();
                        }
                        else{
